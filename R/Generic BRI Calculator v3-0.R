...
#V3.0 calculates BRI scores for samples from the overlapping depth zones by calculating a score from each of the two depth
...
#Lastly, this version is designed to work with a single input file containing both infuana and station information, rather than the
# two file format used in previous versions
...
#      2. input_path - a quoted string detailing the name and location of the .csv file with infauna abundance
#         data and the station information for each infauna sample.
#         Remember with path names, that R use forward slashes "/" not the normal backslash "\" that windows typically uses.
...
offshore_bri_calc_ed14_v3<-function(file_id, input_path, output_path)
  {
require(tidyverse)
require(openxlsx)
require(janitor)
...
####Input files
infauna_n_station.dat <- read.csv(input_path) #the user's infauna and station information to be submitted
...
infauna_n_station.dat.2<-infauna_n_station.dat %>%
  ...
  mutate(stationid=as.character(stationid),
         ...
         sampledate=as_messydate(sampledate),#trying to be flexible with date format given the weirdness of excel vs .CSV vs. R
         sampledate=as_date(sampledate),
         sample_id=paste(stationid, sampledate, replicate, sep="_"),
         stationid=as.character(stationid),
         depth=as.numeric(depth))

#isolating core data to calculate BRI scores
taxa_to_calc<-infauna_n_station.dat.2 %>%
  select(sample_id, stationid, replicate,sampledate,depth, latitude, longitude, taxon, abundance) %>%
...
write.csv(with.pcode, paste(output_path, "/", file_id, " interim file 1 - taxa w assigned pcodes.csv", sep=""), row.names = FALSE)
#
...
write.csv(no.pcode, paste(output_path, "/", file_id, " interim file 2 - taxa w-o pcodes.csv", sep=""), row.names = FALSE)

write.csv(all.4.bri, paste(output_path, "/", file_id, " interim file 3 - taxa and pcodes by sample.csv",sep=""), row.names=FALSE)
...
defaunated<-all.4.bri %>%
  ...
  select(sample_id, stationid, sampledate, replicate, depth) %>%
...
bri_scores.shallow.1<-all.4.bri %>%
  mutate(drop_flag=case_when(sample_id%in%too.deep.samps~1,
                             sample_id%in%defaunated.samps~1,
                             TRUE~0)) %>%
  filter(drop_flag==0)

# we wanted to add a skeleton df to which data are added in case a dataset was wierd and didn't have any shallow pcode taxa
#which would create an empty dataframe and prevent the retention of all other scores at the end
bri_scores.shallow.0<-bri_scores.shallow.1%>%
  distinct(sample_id, stationid, sampledate, replicate, depth)

bri_scores.shallow.2<-bri_scores.shallow.1 %>%
...
bri_scores.shallow.3<- bri_scores.shallow.0%>%
 left_join(., bri_scores.shallow.2, by=c("sample_id", "stationid", "sampledate", "replicate", "depth"))
...
bri_scores.1<-bri_scores.shallow.3 %>%
...
bri_w_station_info<-bri_scores.2 %>% #attaching station information to the BRI scores
  left_join(., distinct(infauna_n_station.dat.2, stationid, latitude, longitude ), by=c("stationid")) %>%
...
dataset_names <- list("final - BRI Scores"=bri_w_station_info, "int_1 - taxa w pcode"=with.pcode,
                      "int_2 - taxa wo pcode"=no.pcode, "int_3 - all taxa by sample"=all.4.bri)
...
bri.outputs.2<-list("final_bri_scores"=bri_w_station_info,
                    ...
                    "int1_taxa_with_pcode"=with.pcode,
                    "int2_taxa_wo_pcode"=no.pcode,
                    "int3_all_taxa_by_sample"=all.4.bri)
